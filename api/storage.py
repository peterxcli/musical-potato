...
class Storage:
    ...
    def __create_block(self):
        for paths in self.block_path:
            logger.warning(f"Creating folder: {paths}")
    ...
    async def file_integrity(self, filename: str) -> bool:
        ...
        if not all(path.isfile(block_file) for block_file in block_files):
            return False
            
        # 2. size of all data blocks must be equal
        sizes = [path.getsize(block_file) for block_file in block_files]
        if len(set(sizes)) != 1:
            return False
            
        # 3. parity block must exist
        parity_path = self.block_path[-1] / filename
        if not path.isfile(parity_path):
            return False
